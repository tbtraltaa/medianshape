...
from medianshape.experiment.deform import rundeforms as run, cases3d
...
def deformdemo3ds(outdir='data', save=True):
    ...
    pdf_file = None
    if save:
        pdf_file = PdfPages('%s/deformdemo3ds.pdf'%outdir)

    fig = plt.figure(figsize=(8,8))
    figcount = 1
    mesh, smesh, simplices, subsimplices, points, lambdas, mus, alphas, is_closed \
    ...
    fig.tight_layout()
   # plt.show()
    run.rundeform3d(smesh, simplices, subsimplices, input_currents, lambdas, mus, alphas, file_doc=pdf_file, save=save, figcount=figcount)
    if save:
        pdf_file.close()
