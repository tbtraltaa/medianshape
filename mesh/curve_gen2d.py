...
class Mesh():
    ...
    def generate_mesh(self):
        ...
        self.mesh_faces = np.asarray(self.mesh.faces)
        self.mesh_elements = np.asarray(self.mesh.elements)
    ...
    def generate_curve(self, func_str=None):
        sample_points = random.sample(self.mesh.points, int(math.sqrt(len(self.mesh.points))))
        sample_points = np.array(sample_points)
        sample_Xs = np.sort(Set(sample_points[:,0]))
    ...
    def find_closest_points(func_str, sample_Xs):
        func_values = Mesh.vectorize_func(sample_Xs) 
        for x in sample_Xs:
            for y in self.mesh_points[mesh_points[:,0]==x][:,1]
